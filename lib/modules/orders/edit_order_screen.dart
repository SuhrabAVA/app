...
class _EditOrderScreenState extends State<EditOrderScreen> {
  ...
  // Флаг, чтобы данные из провайдера загружались один раз
  bool _dataLoaded = false;

  // DocDB instance to interact with universal `documents` table
  ...
  void initState() {
    ...
    final initialManager = template is OrderModel ? (template as OrderModel).manager : '';
  ...
  void didChangeDependencies() {
    ...
    if (!_dataLoaded) {
      ...
      _managerNames = names;
      if (_selectedManager != null && !_managerNames.contains(_selectedManager)) {
        _selectedManager = null;
      }

      final warehouse = context.read<WarehouseProvider>();
      final formItems = warehouse.getTmcByType('Форма');
      final numbers = <int>[];
      for (final t in formItems) {
        final desc = t.description;
        // Выцепляем первую числовую последовательность в описании формы
        final match = RegExp(r'\d+').firstMatch(desc);
        if (match != null) {
          final n = int.tryParse(match.group(0)!);
          if (n != null) numbers.add(n);
        } else {
          final n = int.tryParse(desc);
          if (n != null) numbers.add(n);
        }
      }
      numbers.sort();
      _availableForms = numbers.map((e) => e.toString()).toList();
      _defaultFormNumber = numbers.isNotEmpty ? (numbers.last + 1) : 1;
  ...
  Future<void> _saveOrder() async {
    ...
    if (_stageTemplateId != null &&
      ...
      // Fetch template stages from documents via DocDB
      final tplRow = await _docDb.getById(_stageTemplateId!);
      if (tplRow != null) {
        final tplData = (tplRow['data'] ?? {}) as Map<String, dynamic>;
        final stagesData = tplData['stages'];
        ...
        // Save or update production plan in documents collection 'production_plans'
        final existingPlans = await _docDb.whereEq('production_plans', 'order_id', createdOrUpdatedOrder.id);
        if (existingPlans.isNotEmpty) {
          final planId = existingPlans.first['id'] as String;
          await _docDb.updateById(planId, {
    ...
    // === Обработка формы ===
    // Определяем номер формы: если выбрана старая — берем выбранный, иначе используем следующий номер
    String? _selectedFormNumber;
    if (_isOldForm) {
      _selectedFormNumber = _selectedOldForm;
    } else {
      _selectedFormNumber = _defaultFormNumber.toString();
    }
    if (_selectedFormNumber != null && _selectedFormNumber.isNotEmpty) {
      // При создании новой формы вставляем её в склад (коллекция tmc)
      if (!_isOldForm) {
        try {
          await warehouse.addTmc(
            type: 'Форма',
            description: _selectedFormNumber,
            quantity: 1,
            unit: '',
            supplier: null,
            note: 'Создано для заказа ${createdOrUpdatedOrder.id}',
          );
        } catch (e) {
          // ignore errors to avoid breaking order save
        }
      }
    }
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            if (_isOldForm)
              DropdownButtonFormField<String>(
                value: _selectedOldForm,
                decoration: const InputDecoration(
                  labelText: 'Номер формы',
                  border: OutlineInputBorder(),
                ),
                items: _availableForms
                    .map((f) => DropdownMenuItem(value: f, child: Text(f)))
                    .toList(),
                onChanged: (val) => setState(() => _selectedOldForm = val),
                validator: (value) {
                  if (_isOldForm && (value == null || value.isEmpty)) {
                    return 'Выберите номер формы';
                  }
                  return null;
                },
              )
              ...
              TextFormField(
                initialValue: _defaultFormNumber.toString(),
                decoration: const InputDecoration(
