...
class PersonnelProvider with ChangeNotifier {
  ...
  void addWorkplace({
    ...
    final id = _uuid.v4();
    // Локально добавляем рабочее место
    ...
    notifyListeners();
    // Сохраняем в базу. Игнорируем ошибки, т.к. локальная модель уже обновлена
    unawaited(_supabase.from('workplaces').insert({
      'id': id,
      'name': name.trim(),
      'positionIds': positionIds,
  ...
  Future<void> updateWorkplace({
    ...
    try {
      ...
      await _supabase.from('workplaces').update(updateData).eq('id', id);
