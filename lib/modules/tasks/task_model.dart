...
/// Граница «секунды или миллисекунды» для эпох-меток.
///
/// 2 000 000 000 секунд — это 2033 год, а 2 000 000 000 миллисекунд — 1970-й.
/// Значит любое значение ниже границы заведомо задано в секундах, а выше —
/// в миллисекундах, и так будет ещё несколько лет.
///
/// Единицы в `tasks.comments` смешанные: старые записи лежат в секундах,
/// новые — в миллисекундах. Раньше порог в двух местах был задан как
/// 2 000 000 000 000 (в тысячу раз больше), из-за чего текущее время в
/// миллисекундах (~1.79×10¹²) не дотягивало до границы, ошибочно считалось
/// секундами и домножалось на 1000 — комментарии показывались 58-тысячным
/// годом.
const int kEpochSecondsThreshold = 2000000000;

/// Граница «микросекунды»: значения выше неё приходили из старых записей,
/// где метка сохранялась в микросекундах.
const int kEpochMicrosecondsThreshold = 10000000000000;

/// Приводит эпох-метку к миллисекундам независимо от того, в каких единицах
/// она сохранена.
///
/// Единственный нормализатор в проекте: и отображение комментариев, и расчёт
/// длительностей, и история заказа должны видеть одно и то же значение.
int normalizeEpochToMillis(int ts) {
  if (ts <= 0) return ts;
  if (ts > kEpochMicrosecondsThreshold) return ts ~/ 1000;
  if (ts < kEpochSecondsThreshold) return ts * 1000;
  return ts;
}
