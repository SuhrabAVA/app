...
class _DeletedRecordsScreenState extends State<DeletedRecordsScreen> {
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final deletedBy =
                          (record['deleted_by'] ?? '').toString().trim();
                      final deletedAt =
  ...
  List<Widget> _buildPayloadTiles(Map<String, dynamic> payload) {
    ...
    final quantity = _firstNumber(
        payload,
        const ['quantity', 'qty', 'amount', 'count', 'counted_qty', 'factual'],
        usedKeys);
    final unit =
        _firstString(payload, const ['unit', 'units', 'measure'], usedKeys);
    ...
    final grammageValue =
        _firstString(payload, const ['grammage', 'grammage_display'], usedKeys);
    ...
    final criticalThreshold = _firstNumber(payload,
        const ['critical_threshold', 'critical', 'critical_stock'], usedKeys);
    if (criticalThreshold != null) {
      rows.add(MapEntry(
          'Критический остаток', _formatNumber(criticalThreshold.toDouble())));
    }
    ...
    final otherKeys = payload.keys.where((k) => !skipKeys.contains(k)).toList()
  ...
  String _formatNumber(double value) {
    ...
    return value
        .toStringAsFixed(value.abs() >= 100 ? 1 : 2)
        .replaceAll('.', ',');
  }
