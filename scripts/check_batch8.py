...
out_path = os.path.join(ROOT, "scripts", "batch8_check.txt")
with io.open(out_path, "w", encoding="utf-8") as out:
    rows = get("orders", {"select": "id,customer,manager,order_date,product", "customer": "ilike.*new york*"})
    out.write("=== New York (%d) ===\n" % len(rows))
    for r in rows:
        p = r.get("product") or {}
    ...
    forms = get("forms", {"select": "id,number,series", "number": "eq.1761"})
    out.write("\n=== form 1761 ===\n%s\n" % forms)
print("done")
