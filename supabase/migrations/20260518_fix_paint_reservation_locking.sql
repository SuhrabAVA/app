-- Fix paint reservation RPCs by avoiding FOR UPDATE on nullable outer-join rows.
...
begin
  ...
  loop
    ...
    if not found then
      insert into order_paint_reservations(order_id, paint_id, paint_name, reserved_qty, used_qty, released_qty)
      values (p_order_id, rec.paint_id, coalesce(rec.paint_name, v_stock_name, rec.stock_name), rec.qty, rec.qty, 0)
      on conflict (order_id, paint_id) where paint_id is not null
