...
--
-- СТАТУС
-- Функция применена в базе, и OrderQueueSyncService.sync переключён на неё:
-- состав, seq и step_no плана пишет только этот вызов. Старый путь (отдельные
-- удаления, парковка, обновления и вставки без транзакции) удалён из Dart
-- после инцидента 25.08.2026, когда обрыв запроса сразу после парковки оставил
-- план заказа с seq = -1000000…-1000002: очередь показывалась перевёрнутой, а
-- сохранение падало с «Не удалось создать производственные задания».
-- Клиент теперь передаёт только состав очереди и step_no; физический seq
-- раздаёт функция — только ей видны номера, занятые защищёнными этапами.
--
-- ЗАЧЕМ
